<!DOCTYPE HTML>
<html><head><title>Register member</title> </head>
 <body>
...
//this line is for debugging purposes so that we cna see the actual POST data
...
//save the member data if the error flag is still clear
    if ($error == 0) {
        $query = "INSERT INTO member (firstname,lastname,email,username,password) VALUES (?,?,?,?,?)";
...
<form method="POST" action="registermember.php">
  ...
  <p>
    ...
    <input type="text" id="username" name="username" minlength="8" maxlength="32" required> 
  </p> 
  <p>
